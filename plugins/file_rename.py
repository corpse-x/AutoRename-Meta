...
from helper.database import autorenameDB
...
# Example Usage:
filename = "Naruto Shippuden S01[episode] [quality][Dual Audio] @AnimePlaza_STR.mkv"
...
async def auto_rename_files(client, message):
    ...
    format_template = await autorenameDB.get_format_template(user_id)
    media_preference = await autorenameDB.get_media_preference(user_id)
    ...
    try:
        ...
        _bool_metadata = await autorenameDB.get_metadata(user_id)
        if _bool_metadata:
            metadata = await autorenameDB.get_metadata_code(user_id)
            if metadata:
        ...
        ph_path = None
        c_caption = await autorenameDB.get_caption(message.chat.id)
        c_thumb = await autorenameDB.get_thumbnail(message.chat.id)
