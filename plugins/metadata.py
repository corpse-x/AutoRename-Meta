...
async def handle_metadata(bot: Client, message: Message):
    ...
    bool_metadata = await autorenameDB.get_metadata(message.from_user.id)
    user_metadata = await autorenameDB.get_metadata_code(message.from_user.id)
    await ms.delete()
    if bool_metadata:
...
async def query_metadata(bot: Client, query: CallbackQuery):
    ...
    if data.startswith("metadata_"):
        ...
        user_metadata = await autorenameDB.get_metadata_code(query.from_user.id)

        if bool(eval(_bool)):
            await autorenameDB.set_metadata(query.from_user.id, bool_meta=False)
            ...
            await autorenameDB.set_metadata(query.from_user.id, bool_meta=True)
            await query.message.edit(
                f"<b>Your Current Metadata:</b>\n\n➜ `{user_metadata}` ",
                reply_markup=InlineKeyboardMarkup(ON),
            )
        ...
        try:
            ...
            await autorenameDB.set_metadata_code(
