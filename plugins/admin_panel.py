    ...
    try:
        ...
        await message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("⚡ Tutorial", url="https://t.me/yukiLOGS")]]
            ),
        )
...
async def get_stats(bot, message):
    total_users = await autorenameDB.total_users_count()
...
async def broadcast_handler(bot: Client, m: Message):
    ...
    all_users = await autorenameDB.get_all_users()
    ...
    async for user in all_users:
        ...
        if sts == 200:
            ...
            failed += 1
        if sts == 400:
            await autorenameDB.delete_user(user["_id"])
        done += 1
        if not done % 20:
            await sts_msg.edit(
